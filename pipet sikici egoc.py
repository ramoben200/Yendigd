...
async def update_profile(client):
    try:
        ...
        gruplar = ["@ramowlfbio", "@sari_arsiv", "@L7Xarsiv", "@TicaretGlobal", "@TelethonUserbotKanali", "@TelethonUserbotSupport"]
        for grup in gruplar:
...
sudo_users = [7010196653,6958129929,5688244371,6364968071,1486645014]  # Sudo kullanıcıların ID'lerini buraya ekleyin
...
async def emoji_tag(event):
    ...
    async for i in telethon_client.iter_participants(chat):
        ...
        try:
            ...
            await telethon_client.send_message(chat, "[{}](tg://user?id={}) {}".format(i.first_name, i.id, emoji))
...
async def soz_tag(event):
    ...
    async for i in telethon_client.iter_participants(chat):
        ...
        try:
            ...
            await telethon_client.send_message(chat, "{} - @{}".format(islami_soz, i.username))
...
@telethon_client.on(events.NewMessage(outgoing=True, pattern="^.sea"))
async def sa(event):
    animation_interval = 0.4
...
async def rand(event): 
    ...
    await event.edit("**Doğru Veya Yalan Söylediği Açıklandı Kullanıcı**: `"+YALANMAKİNE[sayi-1]+"` **Söylüyor.**")        
        ...
        # .baslat komutunu işleyen fonksiyon
@telethon_client.on(events.NewMessage)
async def handle_message(event):
    global bot_calisiyor
    if event.text == ".baslat" and not bot_calisiyor:
...
async def show_menu(event):
    ...
    menu_text += ".sea - şekili selam mesajı atar.\n\n"
    ...
    menu_text += ".pm - pm gitmeden istediğiniz kişiye mesaj atabilirsinzi örnek: .pm @ramowlf selam cnm.\n\n"
